...
function data_tabel(tabel) {
    ...
    if (tabel == 'data_transaksi_seller') {
        var xin_table = $('#data_tabel').DataTable({
            ...
            "bFilter": false,
            "ajax": {
                "url": '/data_transaksi_seller',
                "dataType": "json",
                "type": "POST",
                "data": { _token: "{{csrf_token()}}",tgl_start : tgl_start,tgl_end : tgl_end,id_shop : id_shop,status_seller : status_seller,search :search,sort:sort }
            },
            "columns": columns,
            "bDestroy": true
        });
...
$(function() {
    ...
    data_tabel('data_transaksi_seller')
});
