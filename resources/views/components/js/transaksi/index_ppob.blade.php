...
function data_tabel(tabel) {
    ...
    if (tabel == 'data_transaksi_ppob') {
        var xin_table = $('#data_tabel').DataTable({
            ...
            "bFilter": false,
            "ajax": {
                "url": '/data_transaksi_ppob',
                "dataType": "json",
                "type": "POST",
                "data": { _token: "{{csrf_token()}}",tgl_start : tgl_start,tgl_end : tgl_end,id_category : id_category,status : status,search :search,sort:sort }
            },
            "columns": columns,
            "bDestroy": true
        });
...
$(function() {
    ...
    data_tabel('data_transaksi_ppob')
});
