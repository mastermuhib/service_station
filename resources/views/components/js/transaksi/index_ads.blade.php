...
function data_tabel(tabel) {
    ...
    if (tabel == 'data_transaksi_ads') {
        var xin_table = $('#data_tabel').DataTable({
            ...
            "bFilter": false,
            "ajax": {
                "url": '/data_transaksi_ads',
                "dataType": "json",
                "type": "POST",
                "data": { _token: "{{csrf_token()}}",tgl_start : tgl_start,tgl_end : tgl_end,id_ads : id_ads,status : status,search :search,sort:sort }
            },
            "columns": columns,
            "bDestroy": true
        });
