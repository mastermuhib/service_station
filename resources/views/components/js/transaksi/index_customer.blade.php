...
function data_tabel(tabel) {
    ...
    var tgl_end = $("#end").val();
    var status = $("#status_customer").val();
    var status_customer = $("#status_customer").val();
    var search = $("#s_search").val();
    var sort = $("#sort").val();
    
    if (tabel == 'data_transaksi_customer') {
        var xin_table = $('#data_tabel').DataTable({
            ...
            "bFilter": false,
            "ajax": {
                "url": '/data_transaksi_customer',
                "dataType": "json",
                "type": "POST",
                "data": { _token: "{{csrf_token()}}",tgl_start : tgl_start,tgl_end : tgl_end,status : status,status_customer : status_customer,search :search,sort:sort }
            },
            "columns": columns,
            "bDestroy": true
        });
...
$(function() {
    ...
    data_tabel('data_transaksi_customer')
});
